...
const updateSorter = (sortName: string) => {
  ...
  const alreadyActiveSorter: boolean = lastSorter.value.index === sortName
  const order: 'asc' | 'desc' =
    alreadyActiveSorter && lastSorter.value.order === 'asc' ? 'desc' : 'asc'
  if (['name'].includes(sortName)) sorter = sorterCharactere(sortName)
  else if (['market_cap', 'current_price', 'total_volume'].includes(sortName)) { sorter = sorterPrices(currencyActive, sortName) } else if (['sparkline_in_7d'].includes(sortName)) sorter = sorterSparkline7days(currencyActive, sortName)
  else sorter = null
...
<template>
  <div
    class="dyn-order max-w-max flex flex-1"
    style="max-width: 100%"
  >
